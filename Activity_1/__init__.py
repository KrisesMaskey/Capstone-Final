...
doc = """
Baseline Dictator Game (One Proposer)
"""
...
class Player(BasePlayer):
    ...
    player_choice = models.StringField(choices=['A','B',], label="Your Choice")
...
class Instructions(Page):
    ...
    def vars_for_template(player):
        return (dict(
                ...
                B_X1 = player.session.config['baseline_dg_game_payoff']['Action_A']['Player_1'],
                B_Y1 = player.session.config['baseline_dg_game_payoff']['Action_A']['Receiver'],
                B_X2 = player.session.config['baseline_dg_game_payoff']['Action_B']['Player_1'],
                B_Y2 = player.session.config['baseline_dg_game_payoff']['Action_B']['Receiver']
            ))
...
class WaitRoom(Page):
    ...
    @staticmethod
    def vars_for_template(player):
        player.player_role = player.session.config['control_player_role'][0] if (player.session.config['CONTROL_FLAG'] == True) else 'Proposer'
...
class GamePage(Page):
    ...
    @staticmethod
    def vars_for_template(player):
        return dict(A_x = player.session.config['baseline_dg_game_payoff']['Action_A']['Player_1'], A_y = player.session.config['baseline_dg_game_payoff']['Action_A']['Receiver'],
                    B_x = player.session.config['baseline_dg_game_payoff']['Action_B']['Player_1'], B_y = player.session.config['baseline_dg_game_payoff']['Action_B']['Receiver'],
                    form_label= " ",)
    ...
    def before_next_page(player, timeout_happened):
        ...
        if(timeout_happened):
            ...
            if(player.player_choice == 'A'):
                player.payoff = player.session.config['baseline_dg_game_payoff']['Action_A']['Player_1']

            elif(player.player_choice == 'B'):
                player.payoff = player.session.config['baseline_dg_game_payoff']['Action_B']['Player_1']
            
            participant = player.participant
            participant.activity_earning = float(player.payoff)  
            participant.op_nature = ''
...
class SOE(Page):
    ...
    def vars_for_template(player):
        return (dict(showup_fee = player.session.config['showup_fee'], timeout_flag = player.participant.activity_timeout, 
                     activity_one = True, bonus = player.session.config['follow_up_question_bonus'], player_action = player.player_choice, 
                     earning = float(player.payoff), t = float(player.payoff),
                     ))
